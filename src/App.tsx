      ...
      <main className="MainContainer">
        ...
        <InputSelect<Employee>
          ...
          onChange={async (newValue) => {
            //uses the onChange function newValue from the InputSelectFile
            if (newValue === null) {
              return;
            }
            if (newValue.id === "") {
              // added this conditional to deal with if the newValue.id === '' which is when we click all employees
              setNewId("");
        ...
        <div className="RampGrid">
          ...
          {transactions !== null &&
            ...
            (transactionsByEmployee === null ||
              transactionsByEmployee.index + 5 <
                transactionsByEmployee.length) && (
              <button
                className="RampButton"
                disabled={paginatedTransactionsUtils.loading}
                onClick={async () => {
                  if (newId === "") await loadAllTransactions();
                  else await loadTransactionsByEmployee(newId, true);
                }}
              >
                View More
              </button>
